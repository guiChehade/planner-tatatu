const CACHE_NAME = 'planner-intuitivo-v1';
const urlsToCache = [
  '/',
  '/static/js/bundle.js',
  '/static/css/main.css',
...
self.addEventListener('push', (event) => {
  const options = {
    body: event.data ? event.data.text() : 'Nova notificação do Planner Intuitivo',
    icon: '/icon-192.png',
    badge: '/icon-192.png',
    vibrate: [100, 50, 100],
    data: {
  ...
  event.waitUntil(
    self.registration.showNotification('Planner Intuitivo', options)
  );
});
